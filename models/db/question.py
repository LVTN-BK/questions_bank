...
class Questions_Evaluation_DB(BaseModel):
    ...
    correct_probability: float = Field(..., description='correct probability of question')
    num_student: int = Field(..., description='number of student')
    # discrimination_param: float = Field(default=1, description='discrimination param')
    average_ability: float = Field(..., description='average ability of student')
    difficult_value: float = Field(..., description='difficult of item')
